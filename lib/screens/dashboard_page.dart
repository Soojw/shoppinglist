...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<void> _detectUserLocation() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        String city = address['city'] ?? address['town'] ?? address['county'] ?? state;
  ...
  Future<List<PriceDropItem>> _fetchLivePriceData() async {
    ...
    try {
      ...
      debugPrint('Error fetching dashboard CSV data: $e');
    }
    return [];
  }
  ...
  Future<List<PriceDropItem>> _fetchUserFavoritePreviews(String email) async {
    ...
    try {
      ...
      debugPrint('Fav Preview Error: $e');
    }

    return grouped.values.take(2).toList();
  }

  void _showDemoToast(String message) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(message), backgroundColor: const Color(0xFF059669), duration: const Duration(seconds: 1), behavior: SnackBarBehavior.floating));
  }
                ...
                child: Column(
                  ...
                  children: [
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  Text(currentLocation, style: const TextStyle(color: primaryGreen, fontWeight: FontWeight.bold, fontSize: 12)),
                                ],
                              )
                            ],
                          ),
                        ...
                        GestureDetector(
                          onTap: () {
                            if (!userProvider.isLoggedIn) {
                              _showDemoToast('Please Log In to view notifications');
                              return;
                            }
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Row(children: [Text('Item Price Drop', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: deepSlate)), SizedBox(width: 8), Icon(Icons.sensors, color: Colors.redAccent, size: 16)]),
                        TextButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const FullListPage(title: 'Item Price Drop'))), child: const Text('See all', style: TextStyle(color: primaryGreen, fontWeight: FontWeight.bold))),
                      ],
                    ),
                    ...
                    SizedBox(
                      ...
                      child: FutureBuilder<List<PriceDropItem>>(
                        ...
                        builder: (context, snapshot) {
                          ...
                          else if (!snapshot.hasData || snapshot.data!.isEmpty) return const Center(child: Text('No local price drops found.', style: TextStyle(color: Colors.grey)));
                          else {
                    ...
                    Container(
                      ...
                      child: ListTile(
                        ...
                        leading: Container(padding: const EdgeInsets.all(10), decoration: const BoxDecoration(color: Color(0xFFD1FAE5), shape: BoxShape.circle), child: const Icon(Icons.receipt_long, color: primaryGreen)),
                        title: const Text('Smart Shopping List', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                        subtitle: const Text('Calculate lowest total bill for your basket', style: TextStyle(color: Colors.grey, fontSize: 12)),
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        const Text('Your alerts', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: deepSlate)),
                        TextButton(
                          onPressed: () {
                            if (!userProvider.isLoggedIn) { _showDemoToast('Please Log In first'); return; }
                            Navigator.push(context, MaterialPageRoute(builder: (context) => UserAlertsPage(userEmail: userProvider.userEmail)));
                          },
                    ...
                    Container(
                      ...
                      child: !userProvider.isLoggedIn
                          ? const Padding(padding: EdgeInsets.symmetric(vertical: 24.0), child: Center(child: Text('Sign in to view your personalized alerts.', style: TextStyle(color: Colors.grey))))
                          : FutureBuilder<List<PriceDropItem>>(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return Column(
                            children: items.asMap().entries.map((entry) {
                              ...
                              if (idx < items.length - 1) return Column(children: [row, const Divider(height: 32, color: Color(0xFFF1F5F9))]);
                              return row;
                            }).toList(),
                          );
                        },
      ...
      child: Column(
        ...
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(category, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.grey, letterSpacing: 1.0)),
              Container(padding: const EdgeInsets.all(6), decoration: const BoxDecoration(color: Color(0xFFD1FAE5), shape: BoxShape.circle), child: Icon(iconData, color: const Color(0xFF059669), size: 20)),
            ],
          ),
          const SizedBox(height: 8),
          ...
          Row(
            ...
            children: [
              Text(oldPrice, style: const TextStyle(fontSize: 12, color: Colors.grey, decoration: TextDecoration.lineThrough)),
              const SizedBox(width: 8),
              Text(newPrice, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Color(0xFF059669))),
            ],
          ),
        ],
      ),
    ...
    return InkWell(
      ...
      child: Row(
        ...
        children: [
          ...
          const SizedBox(width: 8),
          Text(time, style: const TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
